...
def load_library(library)
    ...
    CSV.foreach(SAVE_FILE, headers: true) do |row|
        case row["type"]
        when "Digital"
            library.add_digital_book(row["title"],row["author"],row["year"].to_i,row["genre"],row["url"])
        when "Audio"
            library.add_audio_book(row["title"],row["author"],row["year"].to_i,row["genre"],row["duration_minutes"].to_i)
        else
            library.add(row["title"],row["author"],row["year"].to_i,row["genre"])
        end
    end
